...
(async () => {
  ...
  function replaceInFile(filePath, searchValue, replaceValue) {
    if (fs.existsSync(filePath)) {
      try {
        ...
        console.log(
          chalk.red(`Error replacing in file ${filePath}: ${error.message}`)
        );
        return false;
      }
    }
  ...
  function findAndReplaceInDirectory(
    ...
    try {
      ...
      console.log(
        chalk.red(`Error searching directory ${directory}: ${error.message}`)
      );
    }
  }
  ...
  function updateIOSSchemes(projectDir, oldName, newName) {
    ...
    const schemesDir = path.join(
      projectDir,
      "ios",
      `${oldName}.xcodeproj/xcshareddata/xcschemes`
    );
    ...
    if (fs.existsSync(schemesDir)) {
      ...
      console.log(
        chalk.yellow(`⚠️ Schemes directory not found: ${schemesDir}`)
      );
    }
  }
  ...
  function updateIOSProjectFiles(projectDir, oldName, newName) {
    ...
    if (fs.existsSync(oldProjectDir) && oldName !== newName) {
      ...
      console.log(
        chalk.green(
          `✅ Renamed project directory: ${oldName}.xcodeproj -> ${newName}.xcodeproj`
        )
      );
    }
    ...
    if (fs.existsSync(oldWorkspaceDir) && oldName !== newName) {
      ...
      console.log(
        chalk.green(
          `✅ Renamed workspace directory: ${oldName}.xcworkspace -> ${newName}.xcworkspace`
        )
      );
    }

    const workspaceContentFile = path.join(
      newWorkspaceDir,
...
`;
  ...
  program
    ...
    .action(async (projectName, options) => {
      console.log(
        chalk.blue("🚀 Creating a new React Native project from template")
      );

      console.log(chalk.yellow(`\n📦 Creating project ${projectName}...`));
      try {
        execSync(
          `git clone https://github.com/linhnguyen-gt/new-react-native.git ${projectName}`,
          {
            stdio: "inherit",
          }
        );
        ...
        if (fs.existsSync("create-rn-with-redux-project")) {
          ...
          console.log(
            chalk.yellow("Removed create-rn-with-redux-project directory")
          );
        }
        ...
        if (fs.existsSync(`${androidSrcDir}/newreactnative`)) {
          ...
          try {
            ...
            execSync(`rm -rf ${androidSrcDir}/newreactnative`, {
              stdio: "inherit",
            });

            console.log(
              chalk.green(`✅ Android package renamed to ${options.bundleId}`)
            );
          } catch (error) {
            console.log(
              chalk.red(`Error copying Android files: ${error.message}`)
            );
          }
        }
        ...
        if (fs.existsSync(`${iosDir}/NewReactNative`)) {
          execSync(`mv ${iosDir}/NewReactNative ${iosDir}/${projectName}`, {
            stdio: "inherit",
          });
        }
        if (fs.existsSync(`${iosDir}/NewReactNative-Staging`)) {
          execSync(
            `mv ${iosDir}/NewReactNative-Staging ${iosDir}/${projectName}-Staging`,
            {
              stdio: "inherit",
            }
          );
        ...
        if (fs.existsSync(`${iosDir}/NewReactNative-Production`)) {
          execSync(
            `mv ${iosDir}/NewReactNative-Production ${iosDir}/${projectName}-Production`,
            {
              stdio: "inherit",
            }
          );
        ...
        if (fs.existsSync(`${iosDir}/NewReactNative.xcodeproj`)) {
          execSync(
            `mv ${iosDir}/NewReactNative.xcodeproj ${iosDir}/${projectName}.xcodeproj`,
            {
              stdio: "inherit",
            }
          );
        ...
        if (fs.existsSync(`${iosDir}/NewReactNative.xcworkspace`)) {
          execSync(
            `mv ${iosDir}/NewReactNative.xcworkspace ${iosDir}/${projectName}.xcworkspace`,
            {
              stdio: "inherit",
            }
          );
        ...
        if (fs.existsSync(podfilePath)) {
          ...
          replaceInFile(
            podfilePath,
            /project 'NewReactNative'/g,
            `project '${projectName}'`
          );
          replaceInFile(
            podfilePath,
            /target 'NewReactNative'/g,
            `target '${projectName}'`
          );
          replaceInFile(
            podfilePath,
            /target 'NewReactNative-Staging'/g,
            `target '${projectName}-Staging'`
          );
          replaceInFile(
            podfilePath,
        ...
        if (fs.existsSync(".expo")) {
          ...
          try {
            ...
            console.log(
              chalk.red(`Error removing .expo directory: ${error.message}`)
            );
          }
        }
        ...
        if (!options.skipInstall) {
          ...
          try {
            ...
            console.log(
              chalk.yellow(
                "Run 'yarn install' or 'npm install' in the project directory."
              )
            );
          }
        } else {
        ...
        try {
          ...
          console.log(
            chalk.yellow("Run 'yarn env:setup' in the project directory.")
          );
        }
        ...
        if (options.repo) {
          try {
            ...
            execSync(`git remote add origin ${options.repo}`, {
              stdio: "inherit",
            });
        ...
        for (const item of tempDirsToRemove) {
          if (fs.existsSync(item)) {
            try {
              ...
              console.log(
                chalk.red(`Error removing ${item}: ${error.message}`)
              );
            }
          }
        }
